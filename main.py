...
# CodeGrade step2
# Replace None with your code
df_zero_emp = pd.read_sql("""SELECT o.*
FROM
    offices o
LEFT JOIN
    employees e ON o.officeCode = e.officeCode
WHERE
    e.employeeNumber IS NULL""",conn)
df_zero_emp
...
ORDER BY
    CAST(p.amount AS REAL) DESC""",conn)
...
HAVING
    AVG(CAST(c.creditLimit AS REAL)) > 90000
ORDER BY
    numCustomers DESC""",conn)
...
# CodeGrade step10
# Replace None with your code
df_under_20 = pd.read_sql("""WITH underperforming_products AS (
    -- Subquery to find products with < 20 unique purchasers
    SELECT
        od.productCode
    FROM
        orderdetails od
...
-- Main query to link those products to the employees who sold them
SELECT DISTINCT
    e.employeeNumber,
    e.firstName,
    e.lastName,
...
FROM
    employees e
JOIN
    offices o_office ON e.officeCode = o_office.officeCode
JOIN
    customers c ON e.employeeNumber = c.salesRepEmployeeNumber
JOIN
...
WHERE
    od.productCode IN (SELECT productCode FROM underperforming_products)
ORDER BY
    e.lastName""",conn)
